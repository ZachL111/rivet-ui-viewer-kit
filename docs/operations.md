...
`rivet-ui-viewer-kit` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for viewer scenarios with bounded scenario files, conflict explanations, and explicit failure cases.
...
The score is deterministic. The current threshold is 169. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 3.
